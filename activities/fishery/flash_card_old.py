...
def activity_double_week(excel_tool: ExcelToolsForActivities,fishery_id, chapterID, icon_name,open_time, wildCardId=None):
    # paymentGiftId_cfg = {
...
def collection_base(excel_tool: ExcelToolsForActivities, fishery_id, collectionChapterId):
    ...
    while cur < fish_count:
        ...
        if mode == 1:
            ...
            instance_object = json_to_instance(json_object=json_object_list[cur], cls=COLLECTION_BASE)
        ...
        instance_object.collectionSeasonId = 1
        if fish_class == 1:
...
def item_main_wild_card(excel_tool: ExcelToolsForActivities, fishery_id, icon_name, wildCardId):
    ...
    return wildCardId
def item_main_language_wild_card(excel_tool: ExcelToolsForActivities, fishery_id, wildCardId):
    # 万能卡
...
def item_main_language(excel_tool: ExcelToolsForActivities,fishery_id, tokenID, activityFisheryName):
    item_main_language_detail = excel_tool.get_table_data_detail(book_name="ITEM_MAIN_LANGUAGE.xlsm")
    key = "tpId"
    # 活动代币
...
def mission_group(excel_tool: ExcelToolsForActivities, TimerId, fishery_id, activityName, imgNameInner,  newNDaysImgName, missionType=None, groupId=None, groupId_three_fish=None):
...
def timer_main(excel_tool: ExcelToolsForActivities, fishery_id, open_time, chapterTimerId=None):
    ...
    json_object_list = excel_tool.get_table_data_object_list_by_key_value(key="name", value=f"{fishery_id}双周闪卡", table_data_detail=timer_main_detail)
    if json_object_list:
        mode = 2
        ...
        chapterTimerId= excel_tool.get_min_value_more_than_start(key=key, table_object_detail=timer_main_detail, start=1150091)
...
def main(excel_tool: ExcelToolsForActivities):
    ...
    open_time = "2025-08-08 00:00:00"  # 活动开始时间 (timer_main)
    ...
    # 配置修改区结束
    collectionChapterId, wildCardId = activity_double_week(excel_tool=excel_tool, fishery_id=fishery_id, chapterID=collectionChapterId, icon_name=icon_name, wildCardId=wildCardId, open_time=open_time)
    ...
    save_main_id(file_name=file_name, id_dict={"collectionChapterId": collectionChapterId, "wildCardId": wildCardId})
