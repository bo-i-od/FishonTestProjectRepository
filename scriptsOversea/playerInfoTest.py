...
def gift_code_test(bp: BasePage):
    if not PlayerSettingPanel.is_btn_giftcode_active(bp):
        ...
        return
    PlayerInfoPanel.click_btn_giftcode_oversea(bp)
    bp.sleep(1)
    # 输入不存在的礼包码
    giftcode = "1234"
    PlayerInfoPanel.set_giftcode_oversea(bp, giftcode)
    giftcode_input = PlayerSettingPanel.get_giftcode(bp)
    compare(giftcode, giftcode_input)
    PlayerInfoPanel.click_btn_confirm_oversea(bp)
    bp.sleep(1)
    bp.cmd("activationcodeclear")
    # 输入存在的礼包码
    giftcode = "fishon888"
    PlayerInfoPanel.set_giftcode_oversea(bp, giftcode)
    giftcode_input = PlayerInfoPanel.get_giftcode_oversea(bp)
    compare(giftcode, giftcode_input)
    PlayerInfoPanel.click_btn_confirm_oversea(bp)
    ...
    bp.sleep(1)
    PlayerInfoPanel.click_btn_close_additional_oversea(bp)

def settings_test(bp: BasePage):
    PlayerInfoPanel.click_tab_avatar_oversea(bp)
    bp.sleep(1)
    PlayerInfoPanel.click_tab_head_frame_oversea(bp)
    bp.sleep(1)
    PlayerInfoPanel.click_tab_name_oversea(bp)
    bp.sleep(1)
    PlayerInfoPanel.click_tab_setting_oversea(bp)
    bp.sleep(1)
    PlayerInfoPanel.set_slider_music_oversea(bp, random.random())
    PlayerInfoPanel.set_slider_sound_oversea(bp, random.random())
    PlayerInfoPanel.set_options_graphics_oversea(bp, random.randint(0, 2))
    PlayerInfoPanel.set_options_frame_oversea(bp, random.randint(0, 1))
    PlayerInfoPanel.set_options_joystick_oversea(bp, random.randint(0, 1))
    PlayerInfoPanel.set_options_vibration_oversea(bp, random.randint(0, 1))
    PlayerInfoPanel.set_options_gyro_oversea(bp, random.randint(0, 1))
#    PlayerInfoPanel.set_options_invite(bp, random.randint(0, 1))
    PlayerInfoPanel.click_btn_copy_id_oversea(bp)

def name_test(bp: BasePage):
    # 将名字改为当前时间
    now = datetime.datetime.now()
    name = 't' + now.strftime("%y%m%d%H%M%S")
    PlayerInfoPanel.set_player_name_oversea(bp, name)

    # 保存
    PlayerInfoPanel.click_btn_save_oversea(bp)
    bp.sleep(1)

    # 二次确认
    MessageBoxPanel.click_btn_confirm(bp)
    bp.sleep(1)
    if not PlayerInfoPanel.is_btn_save_pay_abled_oversea(bp):
        bp.debug_log("erro_"+"if not PlayerInfoPanel.is_btn_save_pay_abled(bp)")

    # 返回上级 对比是否改名成功
    PlayerInfoPanel.click_btn_close_additional_oversea(bp)
    bp.sleep(1)
    playerInfo = PlayerInfoPanel.get_player_info_oversea(bp)
...
def avatar_test(bp: BasePage):
    # 排除当前选中的头像
    avatar_id_list = PlayerInfoPanel.get_avatar_id_list_oversea(bp)
    ...
    bp.sleep(1)
    PlayerInfoPanel.click_btn_close_additional_oversea(bp)
    player_info = PlayerInfoPanel.get_player_info_oversea(bp)
    compare(avatar, player_info["head_img"])

def head_frame_test(bp: BasePage):
    # 排除当前选中的头像
    head_frame_id_list = PlayerInfoPanel.get_head_frame_id_list_oversea(bp)
    # 选择第二个头像进行点击
    random_num = 1
...
def main(bp:BasePage):
    ...
    # 点击鱼竿详情
    PlayerInfoPanel.click_btn_i_rod_oversea(bp)
    bp.sleep(1)
    bp.click_position([0.1, 0.5])

    # 点击勋章再退出
    PlayerInfoPanel.click_btn_edit_achievement_oversea(bp)
    bp.sleep(1)
    PlayerInfoPanel.click_btn_confirm_oversea(bp)
    bp.sleep(1)
...
#    PlayerInfoPanel.switch_tab(bp, 2)
 #   bp.sleep(1)
    ...
    # 点击设置
    PlayerInfoPanel.click_btn_setting_oversea(bp)
    bp.sleep(1)
    settings_test(bp)
    PlayerInfoPanel.click_btn_close_additional_oversea(bp)
    bp.sleep(1)
    ...
    # 点击战力详情
    PlayerInfoPanel.click_btn_i_rating_oversea(bp)
    bp.sleep(1)
    bp.click_position([0.1, 0.5])

    # 改名测试
    PlayerInfoPanel.click_btn_edit_player_info_oversea(bp)
    bp.sleep(1)
    PlayerInfoPanel.click_tab_name_oversea(bp)
    bp.sleep(1)
    name_test(bp)
    ...
    # 点击复制姓名
    PlayerInfoPanel.click_btn_copy_oversea(bp)
    bp.sleep(1)
