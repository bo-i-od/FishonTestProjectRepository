...
def worker(serial_number, task_list, retry_list, pass_list, fail_list, task_lock, contents):
    ...
    while True:
        ...
        if task_list:
            ...
            # if task_name in ['achievementCategoryTest', 'achievementWantedTest', 'rankTest', 'playerInfoTest', 'dlcDownloadTest','battlePassTest', 'energyTest', 'fishCardTest', 'fishAlbumTest', 'gearTest', 'mailTest', 'minitaskTest', 'treasureChestTest', 'duelTest', 'achievementTest', 'taskTest', 'rouletteTest', 'progressRewardsTest', 'careerTest']:
...
def do_test(dev, task, kind, task_list, retry_list, pass_list, fail_list, task_lock, contents):
    bp = gameInit.restart_to_login(dev=dev, package="com.arkgame.fishingmaster")
    print("bp连接成功", dev)
    try:
        task.main(bp)
