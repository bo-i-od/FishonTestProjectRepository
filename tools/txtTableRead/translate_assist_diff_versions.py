...
_find_assist_enter_stack_pairs = {'{': {'{': True, '"': True, "'": True}, '"': {'{': False, "'": False},
                                  "'": {'{': False, '"': False}}
...
def process_line(line, replace_char):
    ...
    if quote_count > 2:
        ...
        new_line = line[:first_quote_idx + 1] + line[first_quote_idx + 1:last_quote_idx].replace('"',
                                                                                                 replace_char) + line[
                                                                                                                 last_quote_idx:]
        return new_line
    else:
    ...
    while searchOneWorldObject:
        ...
        if word in sub_struct_info:
            ...
            if var_info[g_analyse_var_if_custom_key]:
                ...
                doCollectToTrnaslateList(str_content, decl_vars, target_struct_info, current_lang_key, var_collect_list,
                                         None, var_collect_map)
                                ...
                                if vVarName in static_var_map:
                                    if vVarValue != static_var_map[vVarName]:
                                        if vVarName != g_name_key:
                                            logger.error(
                                                f"tb:{tableName} lang:{current_lang_key} id:{id} static_values not match!!!! part name:{vVarName} exist:{static_var_map[vVarName]} new:{vVarValue}.")
                                            all_need_collector_log.append(
                                                f"tb:{tableName} lang:{current_lang_key} id:{id} static_values not match!!!! part name:{vVarName} exist:{static_var_map[vVarName]} new:{vVarValue}.")
                                else:
            ...
            # if word != tableName:
            logger.error(
                f"addToTranslateCheck error:word {word} not in sub_struct_info from map {tableName}, current lang:{current_lang_key}, whole info:{to_search_content}.")
            all_need_collector_log.append(
                f"addToTranslateCheck error:word {word} not in sub_struct_info from map {tableName}, current lang:{current_lang_key}, whole info:{to_search_content}.")
            break
...
def doSubStructString(content):
    ...
    while searchObject:
        ...
        if r_index == -1:
            logger.error(
                f"doSubStructString error found no edge for {vTableName} near: {cut_content[:log_near_char_count]}")
            all_need_collector_log.append(
                f"doSubStructString error found no edge for {vTableName} near: {cut_content[:log_near_char_count]}")
        else:
...
def doCollectToTrnaslateList(content, decl_vars, sub_struct_info, current_lang_key, collect_list, collect_dup_map,
                             collect_map):
    if content and len(content) > 0:
        ...
        while searchObject:
            ...
            if r_index == -1:
                logger.error(
                    f"doCollectToTrnaslateList error found no edge for {vTableName} when collect lang {current_lang_key} at here:{cut_content[0:20]}")
                all_need_collector_log.append(
                    f"doCollectToTrnaslateList error found no edge for {vTableName} when collect lang {current_lang_key} at here:{cut_content[0:20]}")
            else:
                vars_string = cut_content[:r_index]
                trans_struct, new_create = addToTranslateCheck(vTableName, vars_string, decl_vars, sub_struct_info,
                                                               current_lang_key, collect_map)
                # 子类型不添加到全数据列表数组
                # if not g_no_lang_mode:
                # if not sub_struct_info:
...
def doTest():
    # DLC_NODES
    content = readFile(
        "D:\\work\\fishing\\svn\\Release_global\\datapool\\ElementData\\BaseData\\ACHIEVEMENT_WANTED.decl.h", "latin1")
    decl_vars_map = doCollectDeclMap("ACHIEVEMENT_WANTED", content)
    datas_content = readFile(
        "D:\\work\\fishing\\svn\\Release_global\\datapool\\ElementData\\BaseData\\ACHIEVEMENT_WANTED.data.txt")
    collect_list = []
    collect_map = {}
...
def doCollectDeclMap(table_name_up, content):
    ...
    if content and len(content) > 0:
        ...
        while searchObject:
            ...
            if r_index == -1:
                logger.error(
                    f"doCollectDeclList error found no edge for {vTableName} near: {cut_content[:log_near_char_count]}")
                all_need_collector_log.append(
                    f"doCollectDeclList error found no edge for {vTableName} near: {cut_content[:log_near_char_count]}")
            else:
                vars_string = cut_content[:r_index]
...
def doWorkOnDataTxtFile(file_ab_path, current_lang_key, collect_list, collect_dup_map, collect_map):
    ...
    if decl_vars and len(decl_vars) > 0:
        ...
        if content and len(content) > 0:
            doCollectToTrnaslateList(content, decl_vars, None, current_lang_key, collect_list, collect_dup_map,
                                     collect_map)
    else:
...
def doCollectFromMotherDatas(table_classify: ClassifyFile, mother_en_path, current_lang_key, collect_list,
                             collect_dup_map, collect_map, on_table_names_map=None):
...
def doWorkCollectOnFolderAndExport(table_classify, mother_en_path, other_lang_path, collect_list, collect_dup_map,
                                   collect_map, withTranslated=False, on_table_names_map=None):
    '''从母表中收集多语言，并导出为excel'''
    doCollectFromMotherDatas(table_classify, mother_en_path, g_en_key, collect_list, collect_dup_map, collect_map,
                             on_table_names_map)
    if withTranslated:
...
def doCollectAllLanguagesFromElementDataPath(path):
    ...
    doWorkCollectOnFolderAndExport(table_classify, mother_en_path, other_lang_path, collect_list, None, collect_map,
                                   withTranslated)
...
def doWorkReadTranslatedExcels(container_path):
    ...
    for item in files_and_folders:
        ...
        if os.path.isfile(full_path) and (
                item.endswith(".xlsx") and (item.startswith("translated") or item.startswith("to_translate"))):
                    ...
                    if dup_key != main_key:
                        ...
                        if dup_table_name in version_map:
                            ...
                            if list_assist_key in list_assist:
                                ...
                                list_assist_struct = {g_table_name_key: dup_table_name, g_id_key: dup_id,
                                                      g_sort_key: dup_part}
...
def doCompareThreeVersionDetails(old_510, china_712, new_810, write_path, old_version_path, china_712_path,
                                 new_version_path):
    new_to_translate = []
                ...
                for part in id_data:
                    ...
                    if lang_810_str != lang_510_str:
                        ...
                        if lang_510_str == lang_712_str:
                            ...
                            export_both_modified_list.append(
                                {g_table_name_key: table_name, g_id_key: id, g_sort_key: part,
                                 part: {g_old_510_key: lang_510_str, g_china_712_key: lang_712_str,
                                        g_global_810_key: lang_810_str}})
                            pass
    else:
        # 遍历 810版本最新内容
                    ...
                    if g_no_lang_mode:
                        ...
                        if lang_712_str != lang_510_str:
                            ...
                            if lang_810_str == lang_510_str:
                                # 但 810和712无差异
                                # 都需要进行本地化，导出另一个独立excel
                                new_to_translate.append(
                                    {g_table_name_key: table_name, g_id_key: id, g_sort_key: part, part: part_info})
                                pass
                        else:
                            if lang_810_str != lang_510_str:
                                write_back_struct = None
    ...
    writeExportBothModifiedListToExcel3(write_path + os.sep + "manual.xlsx", export_both_modified_list,
                                        old_version_path, china_712_path, new_version_path)
...
def doCompareTwoVersionDetails(old_510, new_810, write_path):
                ...
                if part != g_table_name_key and part != g_id_key and part != g_static_var_key:
                    ...
                    if g_no_lang_mode:
                        ...
                        if lang_810_str != lang_510_str:
                            ...
                            export_both_modified_list.append(
                                {g_table_name_key: table_name, g_id_key: id, g_sort_key: part,
                                 part: {g_old_key: lang_510_str, g_new_key: lang_810_str}})
                            pass
                    else:
                        # 多语言时
                        table_name = table_info[g_table_name_key]
    ...
    export_both_modified_list = sortVersionList(export_both_modified_list)
    writeExportBothModifiedListToExcel2(write_path + os.sep + "manual2.xlsx", export_both_modified_list, '旧版本值',
                                        '新版本值')
...
def writeBackNormalTableToVersion(version_path, exported_map, version_map):
    ...
    for table_name in all_tables:
        if table_name in version_map:
            ...
            logger.warning(
                f"writeBackNormalTableToVersion warning : no table name {table_name} in target path {version_path}, this table write back failed.")
    ...
    if content and len(content) > 0:
        ...
        while searchObject:
            ...
            if r_index == -1:
                ...
                vars_string = cut_content[:r_index]
                replace_str = searchObject.group() + doWriteBackToStructContent(vTableName, vars_string, exported_map,
                                                                                lang)
                                ...
                                if lang in part_info:
                                    if part in data:
                                        ...
                                        if lang in exist_part:
                                            ...
                                            logger.warning(
                                                f"toreplace table:{table_name} id:{id} part:{part} has no lang:{lang}[{g_code_chinese[lang]}]")
                        # 新增
                        for part in data:
                            if part != g_id_key and part != g_table_name_key and part != g_static_var_key:
                                part_info = data[part]
                    ...
                    if part != g_id_key and part != g_table_name_key:
                        if part == g_static_var_key:
                            ...
                            if lang in exist_part:
                                ...
                                new_data[part] = ""
                                logger.warning(
                                    f"tofix: table:{table_name} id:{id} part:{part} has no lang:{lang}[{g_code_chinese[lang]}]")
...
def writeExportBothModifiedListToExcel3(target_output_path, data_list, old_version_path, china_712_path,
                                        new_version_path):
    workbook = xlsxwriter.Workbook(target_output_path)
    sheet = workbook.add_worksheet()
    # 第一行表头
...
def doCompareThreeVersions():
    ...
    # 比校差异，并输出
    doCompareThreeVersionDetails(old_510_version_map, china_712_version_map, new_810_version_map, g_china_712_path,
                                 g_old_version_path, g_china_712_path, g_new_version_path)
    pass
...
def doMergeChsToEnBase():
    ...
    if len(manual_deal_results) > 0:
        writeExportBothModifiedListToExcel2(path + os.sep + "manual_chs.xlsx", export_manual_deal_list, '原始英文表值',
                                            '中文表值')
    else:
...
def doCheckFormatSerial():
    ...
    collect_list = []
    collect_map = {}
    doWorkCollectOnFolderAndExport(ClassifyFile.LangTables, mother_en_path, other_lang_path, collect_list, None,
                                   collect_map, True)
        ...
        if g_table_name_key in table_info and g_id_key in table_info and len(table_info) > 2:
            for part in table_info:
                if part != g_table_name_key and part != g_id_key and part != g_static_var_key:
                    ...
                    doCheckErrorAndWarningFormat(g_chs_key, table_info[g_table_name_key], table_info[g_id_key], part,
                                                 lang_map, error_list, warning_list)
            ...
            if key != g_table_name_key and key != g_id_key and key != g_static_var_key and key != g_sort_key:
                ...
                for other_lang in all_values:
                    if other_lang != main_key:
                        ...
                        other_lang_i = other_lang_i + 1
                        sheet.write(get_column_serial("D", other_lang_i) + str(sheet_line_index),
                                    all_values[other_lang])
    ...
    if len(format_map) > 0:
        for lang_key in lang_map:
            if lang_key != base_key:
                ...
                if g_count_key not in l_format_map or format_map[g_count_key] != l_format_map[g_count_key] or \
                        format_map[g_serial_key] != l_format_map[g_serial_key]:
                    # 数量不对，一定有问题
                    # 顺序不一致，一定有问题
                    error_list.append({g_table_name_key: table_name, g_id_key: table_id,
                                       part: {g_chs_key: var_base_str, lang_key: l_str}})
                else:
                    if format_map[g_count_key] > 1 and format_map[g_all_same_key]:
                        warning_list.append({g_table_name_key: table_name, g_id_key: table_id,
                                             part: {base_key: var_base_str, lang_key: l_str}})
    pass
                        ...
                        if part in en_table_data:
                            ...
                            if has_chinese(en_value):
                                if en_value != chs_value:
                                    manual_deal_results.append(
                                        {g_table_name_key: table_name, g_id_key: table_id, g_sort_key: part,
                                         part: {g_old_key: en_value, g_new_key: chs_value}})
                                    # 双方都是中文时，也回写
                                    #######################################
                            ...
                            logger.warning(
                                f"BaseData table:{table_name} id:{table_id} has no part:{part}, which chs has.")
...
def doCollectAllTableId():
    ...
    doWorkCollectOnFolderAndExport(table_classify, mother_en_path, other_lang_path, collect_list, collect_dup_map,
                                   collect_map, withTranslated)
                ...
                if part != g_table_name_key and part != g_id_key and part != g_static_var_key:
                    ...
                    for lang in g_read_langs:
                        if lang != g_en_key and g_en_key in part_info and lang not in part_info:
                            ...
                            logger.info(
                                f"do add part for {table_name} {id} {part} fill en to {lang}, content:{part_info[g_en_key]}")
    ...
    for lang in part_info:
        ...
        if each_count == -1:
            ...
            if each_count != f_char_count:
                info = {g_table_name_key: table_name, g_id_key: id, g_part_key: part, g_value_key: part_info,
                        part: f'{table_name} {id} {part} %count {lang}:{lang_str} count:{f_char_count} not match {first_count_lang}:{first_lang_str} count:{each_count}'}
                info["excel_data"] = {g_table_name_key: table_name, g_id_key: id,
                                      part: {first_count_lang: first_lang_str, lang: lang_str}}
...
def doScanTranslate():
    ...
    for error_data in error_list:
        logger.error(
            f"{error_data[g_table_name_key]} {error_data[g_id_key]} {error_data[g_part_key]} msg:{error_data[error_data[g_part_key]]}")
    ...
    if len(g_op_files) > 0:
        ...
        if len(arr) > 0:
            for file_path in arr:
                ...
                withTranslated = True
                table_classify = ClassifyFile.LangTables
                doWorkCollectOnFolderAndExport(table_classify, mother_en_path, other_lang_path, collect_list,
                                               collect_dup_map, collect_map, withTranslated, collect_table_names_map)
...
def get_opt():
    try:
        options, args = getopt.getopt(sys.argv[1:], "",
                                      ["in=", "op=", "files=", "help", "old=", "new=", "china=", "notlangtable="])
