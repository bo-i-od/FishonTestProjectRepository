...
class EventsGiftCenterPanel(BasePage):
    def click_btn_close(self):
        self.click_until_disappear(element_data=ElementsData.EventsGiftCenterPanel.btn_close)

    def is_panel_active(self):
        return self.exist(element_data=ElementsData.EventsGiftCenterPanel.EventsGiftCenterPanel)

    def switch_tab(self, index=-1):
        self.click_object_of_plural_objects(element_data=ElementsData.EventsGiftCenterPanel.tab_list, element_viewport=ElementsData.EventsGiftCenterPanel.viewport, viewport_direction="column", index=index)
