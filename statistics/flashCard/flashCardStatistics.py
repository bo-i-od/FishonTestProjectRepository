...
cast_log=load_log_new('../statistics/new_cast_log.txt')
hook_log=load_log_new('../statistics/new_cast_log.txt')

for i in cast_log:
    debug_info=i['debugInfos']
    drop_rate=debug_info['flashCardFinalDropRate']
    if drop_rate>0:
        print()
...
#                     flash_card[f_fish_id]['first_collect_energy_cost']=cast_log[index]["energyCost"]
