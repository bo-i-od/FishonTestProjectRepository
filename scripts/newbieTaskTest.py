...
def day_test(bp: BasePage):
    ...
    if collect_list:
        ...
        stock_expect_list = bp.get_item_count_list(item_icon_name_list=task_reward_icon_list)
        cur = 0
        while cur < len(stock_expect_list):
        ...
        # 对照库存
        stock_list = bp.get_item_count_list(item_icon_name_list=task_reward_icon_list)
        compare_list(stock_expect_list, stock_list)
...
def progress_test(bp:BasePage):
    # 让刚好有三个奖励可领取
    bp.set_item_count(target_count=20000, item_tpid="209001")
    ...
    # 只领取大奖
    # 求期望大奖数量和库存
    bp.set_item_count(target_count=max_reward_threshold, item_tpid="209001")
