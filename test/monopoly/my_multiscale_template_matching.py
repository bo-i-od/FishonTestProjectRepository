...
from airtest.aircv.utils import check_source_larger_than_search, img_mat_rgb_2_gray, generate_result, print_run_time
from airtest.utils.logger import get_logger
from airtest.aircv.error import TemplateInputError
from airtest import aircv

from test.monopoly.my_cal_confidence import cal_rgb_confidence, cal_ccoeff_confidence, cal_rgb_confidence_with_alpha, cal_ccoeff_confidence_with_alpha
...
class MultiScaleTemplateMatching(object):
    ...
    def __init__(self, im_search, im_source, threshold=0.8, rgb=True, record_pos=None, resolution=(), scale_max=800,
                 scale_step=0.005, strict_brightness=False):
        ...
        # 检查模板是否有透明通道
        self.has_alpha = len(im_search.shape) == 3 and im_search.shape[2] == 4
        self.strict_brightness = strict_brightness
    ...
    def find_best_result(self):
        ...
        if self.has_alpha:
            # 提取RGB部分用于模板匹配
            search_rgb = self.im_search[:, :, :3]
            alpha_mask = self.im_search[:, :, 3]
        else:
            search_rgb = self.im_search
            alpha_mask = None

        # 校验图像输入
        check_source_larger_than_search(self.im_source, search_rgb)

        # 第二步：计算模板匹配的结果矩阵res
        s_gray, i_gray = img_mat_rgb_2_gray(search_rgb), img_mat_rgb_2_gray(self.im_source)

        # 如果有透明通道，创建灰度掩码
        mask_gray = None
        if self.has_alpha:
            mask_gray = (alpha_mask > 0).astype(np.uint8) * 255
        ...
        confidence, max_loc, w, h, _ = self.multi_scale_search(
            ...
            threshold=self.threshold, mask=mask_gray)
        ...
        return best_match, confidence if confidence >= self.threshold else None
    ...
    def _get_confidence_from_matrix(self, max_loc, w, h):
        ...
        if self.rgb:
            ...
            if self.has_alpha:
                confidence = cal_rgb_confidence_with_alpha(resized_crop, self.im_search, self.strict_brightness)
            else:
                confidence = cal_rgb_confidence(resized_crop, self.im_search, self.strict_brightness)
        else:
            img_crop = self.im_source[max_loc[1]:max_loc[1] + h, max_loc[0]: max_loc[0] + w]
            resized_crop = cv2.resize(img_crop, (sch_w, sch_h))

            if self.has_alpha:
                confidence = cal_ccoeff_confidence_with_alpha(resized_crop, self.im_search, self.strict_brightness)
            else:
                confidence = cal_ccoeff_confidence(resized_crop, self.im_search, self.strict_brightness)
    ...
    @staticmethod
    def _resize_by_ratio(src, templ, ratio=1.0, templ_min=10, src_max=800, mask=None):
        """根据模板相对屏幕的长边 按比例缩放屏幕"""
        # 截屏最大尺寸限制
        ...
        resized_mask = None
        if mask is not None:
            resized_mask = cv2.resize(mask, (max(int(tw * tr), 1), max(int(th * tr), 1)))

        return src, templ, tr, sr, resized_mask
    ...
    def multi_scale_search(self, org_src, org_templ, templ_min=10, src_max=800, ratio_min=0.01,
                           ratio_max=0.99, step=0.01, threshold=0.8, time_out=3.0, mask=None):
        """多尺度模板匹配"""
        mmax_val = 0
        max_info = None
        r = ratio_min
        t = time.time()
        while r <= ratio_max:
            src, templ, tr, sr, resized_mask = self._resize_by_ratio(
                org_src.copy(), org_templ.copy(), r, src_max=src_max, mask=mask)
            if min(templ.shape) > templ_min:
                ...
                if resized_mask is not None:
                    result = cv2.matchTemplate(src, templ, cv2.TM_CCOEFF_NORMED, mask=resized_mask)
                else:
...
class MultiScaleTemplateMatchingPre(MultiScaleTemplateMatching):
    ...
    def find_best_result(self):
        ...
        if self.resolution != ():
            ...
            # 第一步：校验图像输入
            check_source_larger_than_search(self.im_source, search_rgb)
            if self.resolution[0] < search_rgb.shape[1] or self.resolution[1] < search_rgb.shape[0]:
                raise TemplateInputError("error: resolution is too small.")
            # 第二步：计算模板匹配的结果矩阵res
            if not self.record_pos is None:
                area, self.resolution = self._get_area_scope(self.im_source, search_rgb, self.record_pos, self.resolution)
                self.im_source = aircv.crop_image(self.im_source, area)
                check_source_larger_than_search(self.im_source, search_rgb)
            r_min, r_max = self._get_ratio_scope(
                self.im_source, search_rgb, self.resolution)
            s_gray, i_gray = img_mat_rgb_2_gray(search_rgb), img_mat_rgb_2_gray(self.im_source)
            ...
            confidence, max_loc, w, h, _ = self.multi_scale_search(
                ...
                threshold=self.threshold, time_out=1.0, mask=mask_gray)
            if not self.record_pos is None:
    ...
    def _get_area_scope(self, src, templ, record_pos, resolution):
        ...
        return area, (w * (area[3] - area[1]) / H, h * (area[2] - area[0]) / W)
